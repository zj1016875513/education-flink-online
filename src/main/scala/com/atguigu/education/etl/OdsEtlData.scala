...
object OdsEtlData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    env.setStreamTimeCharacteristic(TimeCharacteristic.ProcessingTime) //设置时间模式为事件时间

    //checkpoint设置
    env.enableCheckpointing(10000l) //10秒做一次checkpoint
    val checkpointConfig = env.getCheckpointConfig
    checkpointConfig.setCheckpointingMode(CheckpointingMode.EXACTLY_ONCE) //仅仅一次
    checkpointConfig.setMinPauseBetweenCheckpoints(5000l) //设置checkpoint间隔时间5秒
